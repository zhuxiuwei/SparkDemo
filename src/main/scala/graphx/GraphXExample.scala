...
object GraphXExample {
  def main(args: Array[String]) {
    ...
    val graph: Graph[(String, Int), Int] = Graph(vertexRDD, edgeRDD)
    	  println( " originGraph vertex partitions count: " + graph.vertices.partitions.length + " , edge partitions count: " + graph.edges.partitions.length)
    ...
    for (triplet <- graph.triplets.collect) {
      ...
      //println(s"${triplet.srcAttr} ${triplet.srcId} ${triplet.dstAttr} ${triplet.dstId} ${triplet.attr}")  //sample: (Bob,27) 2 (Alice,28) 1 7
    }
    ...
    //inDegrees.collect().foreach(print _)  //(1,2)(2,2)(3,1)(4,1)(6,2). which is (vertexId, inDegreeCount)
    ...
    //userGraph.edges.collect().foreach(print _)  //Edge(2,1,7)Edge(2,4,2)Edge(3,2,4)Edge(3,6,3)Edge(4,1,1)Edge(5,2,2)Edge(5,3,8)Edge(5,6,3)
    //userGraph.vertices.collect().foreach(print _) //(1,User(Alice,28,2,0))(2,User(Bob,27,2,2))(3,User(Charlie,65,1,2))(4,User(David,42,1,1))(5,User(Ed,55,0,3))(6,User(Fran,50,2,0))
    //userGraph.triplets.collect().foreach(print _)  //((2,User(Bob,27,2,2)),(1,User(Alice,28,2,0)),7)((2,User(Bob,27,2,2)),(4,User(David,42,1,1)),2)((3,User(Charlie,65,1,2)),(2,User(Bob,27,2,2)),4)((3,User(Charlie,65,1,2)),(6,User(Fran,50,2,0)),3)((4,User(David,42,1,1)),(1,User(Alice,28,2,0)),1)((5,User(Ed,55,0,3)),(2,User(Bob,27,2,2)),2)((5,User(Ed,55,0,3)),(3,User(Charlie,65,1,2)),8)((5,User(Ed,55,0,3)),(6,User(Fran,50,2,0)),3)
...
//    val olderstFollowerInfo:VertexRDD[String] = userGraph.vertices.leftJoin(oldestFollower) { (id, user, optOldestFollower) =>
//      optOldestFollower match {
//        case None => s"${user.name} does not have any followers."
//        case Some((name, age)) => s"${name} is the oldest follower of ${user.name}."
//      }
//    }
//    olderstFollowerInfo.collect.foreach { case (id, str) => println(str) }
//
//    println("Find the oldest follower for each user 2 -- use aggregateMessages============")
//    val olderstFollowers: VertexRDD[User] = userGraph.aggregateMessages[User](
//        edgeCtx => edgeCtx.sendToDst(edgeCtx.srcAttr),
//        (followerA, followerB) => if(followerA.age > followerB.age) followerA else followerB,
//        TripletFields.Src)
//    val olderstFollowersRes = userGraph.vertices.leftJoin(olderstFollowers){
//      (vertexId, user, oldestUserOpt) => (user, oldestUserOpt);
//    }
//    olderstFollowersRes.collect().foreach(x => println(x._2._2.getOrElse("NOBODY") + " is the oldest follower of " + x._2._1.name))
//
...
//      case (id, user, comp) => s"${user.name}: ${id} is in component ${comp.get}"
//    }.collect.foreach { case (id, str) => println(str) }
//
//    println("compute the strongly connected components:============")
//    // compute the strongly connected components
//    val scc = olderGraph.stronglyConnectedComponents(100)
//    // display the component id of each user:
//    olderGraph.vertices.leftJoin(scc.vertices) {
//      case (id, user, comp) => s"${user.name}: ${id} is in strongly connected component ${comp.get}"
//    }.collect.foreach { case (id, str) => println(str) }
